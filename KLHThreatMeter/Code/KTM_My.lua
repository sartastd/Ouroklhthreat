...
me.myevents = { "UI_ERROR_MESSAGE", "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE", "CHAT_MSG_SPELL_AURA_GONE_SELF", 
	"CHAT_MSG_SPELL_ITEM_ENCHANTMENTS", "CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS", "CHAT_MSG_SPELL_SELF_BUFF", "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE",}
...
me.parserstagetwo = 
{
	["debuffstart"] = function(buff)	
		...
		elseif buff == mod.string.get("boss", "spell", "Horreb") then
			me.setstate("Horreb", true)
	...
	["buffend"] = function(buff)
		...
		elseif buff == mod.string.get("spell", "Horreb") then
			me.setstate("Horreb", false)	
...
me.redoglobalthreat = function()
	...
	if me.states["fungalbloom"].value == true then
		me.modifyglobalthreat(-0.999999, mod.string.get("boss", "spell", "fungalbloom"))
	...
	elseif mod.my.class == "druid" then
		stance = me.getstanceindex()
		local rankdruid = 0
		rankdruid = mod.data.gettalentrank("feralinstinct")
			
		
		
		if stance == 1 and rankdruid > 0 then
			
			-- ancien calcul bugguer 1,495
			...
			--Nouveau calcul sartas NG 2022!!
			me.modifyglobalthreat(0.45,mod.string.get("threatmod", "bearform"))
