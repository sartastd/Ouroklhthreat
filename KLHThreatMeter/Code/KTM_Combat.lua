...
me.specialattack = function(abilityid, target, damage, iscrit, spellschool)
	...
	elseif abilityid == "heroicstrike" then
			...
			me.event.damage = damage
			me.event.threat = threatmodifier * (damage + mod.my.ability("heroicstrike", "threat"))
	...
	elseif abilityid == "maul" then
			...
			me.event.damage = damage
			me.event.threat = threatmodifier * (damage + 207)
			me.event.rage = mod.my.ability("maul", "rage") + whitedamage / (UnitLevel("player") / 2)
				
				if mes_sartas_maul then
					local varthreat = threatmodifier * (damage)
					local basethreat = 207*threatmodifier
					mod.out.printtrace(string.format("Threatmodifier : "..threatmodifier.."  Savage Fury : "..(1 + mod.my.mods.druid.savagefury).."  Maul damage : "..damage.."  Variable : "  ..varthreat.."  Base : "..basethreat.. "  Threat : "..math.floor(me.event.threat).."---- By sartas Calculator----"))
	...
	elseif mod.my.class == "warlock" then
		...
		if mod.my.mods.warlock.plagueheart == true then
			...
			-- 2) 25% less for some dots     Plagueheart 6 Bonus
...
me.normalattack = function(spellname, spellid, damage, isdot, target, iscrit, spellschool)
	...
	if mod.my.class == "warlock" then
		...
		if mod.my.mods.warlock.plagueheart == true then
			...
			-- 2) 25% less for some dots     Plagueheart 6 Bonus
			--elseif mod.data.spellmatchesset("Plagueheart 6 Bonus", spellname, nil) == true then
			elseif spellid == "dot" and ((spellname == "Corruption") or (spellname == "Curse of Agony") or (spellname == "Immolate") or (spellname == "Siphon Life")) then
			--elseif mod.data.spellmatchesset("Plagueheart 6 Bonus", abilityid, nil) == true then
				me.event.threat = me.event.threat * 0.75
